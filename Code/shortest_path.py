...
def graph(num_nodes, links, optimized):
    # plot the graph
    ...
    node_col = ['gray' if not node in sp else 'green' for node in graph.nodes()]
    
    # If the edge is in the shortest path set it to red, else set it to white color
    edge_col = ['white' if not edge in red_edges else 'green' for edge in graph.edges()]
    ...
    nx.draw_networkx_edge_labels(graph, node_pos, edge_color = edge_col, edge_labels = optimized)
...
while(len(line)):
    ...
    drive_times[from_node, to_node] = drive_time_arc + construction_arc
...
if user_input == 4:
    ...
    m.setObjectiveN(cost_objective, index = 1, priority = 1, weight = 1, name = 'Cost')
    m.setObjectiveN(driving_objective, index = 2, priority = 1, weight = 1, name = 'Drive Time')
...
# graph the optimized function with others as side constraints
sc_miles = 0
sc_cost = 0
sc_drive_times = 0

if user_input == 1:
    graph(num_nodes, links, miles)
    
    for i,j in links:
        if(x[i, j].x > 0):
            sc_miles += miles[i, j]
            sc_cost += cost[i, j]
            sc_drive_times += drive_times[i, j]
    
    print('Miles (Optimized): ', sc_miles)
    print('Cost: ', sc_cost)
    print('Drive Times: ', str(timedelta(minutes=sc_drive_times))[:-3])
    print('\n\n')
    
if user_input == 2:
    graph(num_nodes, links, cost)
    ...
    print('Cost (Optimized): ', sc_cost)
    print('Miles: ', sc_miles)
    print('Drive Times: ', str(timedelta(minutes=sc_drive_times))[:-3])
    print('\n\n')
    
if user_input == 3:
    graph(num_nodes, links, drive_times)
    ...
    print('Drive Times(Optimized): ', str(timedelta(minutes=sc_drive_times))[:-3])
    print('Miles: ', sc_miles)
    print('Cost: ', sc_cost)
    print('\n\n')
